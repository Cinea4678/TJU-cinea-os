...
lazy_static! {
    static ref RESOLVED_CURSOR: RwLock<Vec<(usize, usize, Rgb888)>> = { RwLock::new(resolve_32rgba(CURSOR)) };
}
...
impl MouseCursor {
    ...
    pub fn print_manually(&self) {
        ...
        GL.read()[pos]
            .lock()
            .display_resolved(self.x as usize, self.y as usize, &RESOLVED_CURSOR.read());
    }
    ...
    pub fn update_mouse(&mut self) {
        ...
        if dx != 0 || dy != 0 {
            ...
            GL.read()[pos]
                .lock()
                .clear_resolved(self.last_x as usize, self.last_y as usize, &RESOLVED_CURSOR.read());
            GL.read()[pos]
                .lock()
                .display_resolved(self.x as usize, self.y as usize, &RESOLVED_CURSOR.read());
            self.last_x = self.x;
