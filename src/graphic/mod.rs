...
// 相关常量
lazy_static! {
    pub static ref BUFFER: Mutex<&'static mut Buffer> = {
        Mutex::new(unsafe { &mut *(Page::<Size4KiB>::containing_address(VirtAddr::new(0xC000_0000)).start_address().as_mut_ptr() as *mut Buffer) })
    };
}
...
impl Writer {
    ...
    pub unsafe fn display_pixel_rgb888(&mut self, x: usize, y: usize, color: Rgb888) {
        BUFFER.lock().chars[x][y].write(Pixel {
            b: color.r(),
            g: color.g(),
            r: color.b(),
        });
...
pub fn display_rect(x: usize, y: usize, w: usize, h: usize, color: u32) {
    let x_end = min(x + h, HEIGHT);
    let y_end = min(y + w, WIDTH);
    qemu_print(format!("{},{},{},{}\n", x, y, x_end, y_end).as_str());
    for i in x..x_end {
        for j in y..y_end {
            unsafe { display_pixel(i, j, color); };
        }
    }
}
...
/// 测试函数：画一幅图
pub fn display_img(x: usize, y: usize, bmp_data: &[u8]) {
    match Bmp::<Rgb888>::from_slice(bmp_data) {
        Ok(bmp) => {
            for Pixel(position, color) in bmp.pixels() {
                unsafe { display_pixel_rgb888(x + position.y as usize, y + position.x as usize, color); };
            }
        }
        Err(error) => {
            qemu_print(format!("{:?}\n", error).as_str());
